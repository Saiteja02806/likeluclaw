...
files = glob.glob("/opt/claw-platform/users/*/config/**/IDENTITY.md", recursive=True)
print(f"Found {len(files)} IDENTITY.md files")
...
for fpath in files:
    ...
    if "gmail-tools/" not in content and "calendar-tools/" not in content:
        print(f"  SKIP (already updated): {fpath}")
        continue
